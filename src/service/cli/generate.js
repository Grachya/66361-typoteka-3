...
const MOCK_COUTN_RESTRICT = 1000;
...
const getAnnounce = () => {
    ...
    const randomTextCount = getRandomInt(MIN_ANNOUNCE_RESTRICT, MAX_ANNOUNCE_RESTRICT);
    return Array(randomTextCount).fill('').map(() => getRandFromArr(ANNOUNCES)).join(' ');
};

const getFullText = () => {
    const randomCount = getRandomInt(1, ANNOUNCES.length - 1);
    return Array(randomCount).fill('').map(() => getRandFromArr(ANNOUNCES)).join(' ');
};
